...
* `--early-stop`: Stop a focus level early once the policy converges (greedy showcase mastery streak or return plateau after clears), then proceed to the next level. Still respects the configured max cycle budget. Thresholds live in `intelligence/config.toml` (`early_stop_*`).
