...
- **Spike-Floored Pits**: In `finalize_sketch_dict`, each pit span becomes a shaft of sampled depth (`pit_min_depth_tiles`–`pit_max_depth_tiles`, default 1–5) measured below the lower floor edge (keeping spikes clear of the jump-arc envelope), ending in a flat **full-width** row of `spike_trap` hazards on solid ground — extremity corner cells included, since `AttachedTile` orientation priority resolves them "top" (pointing up with the row) as long as the shaft walls stay clean. Pits are deadly via spikes rather than bottomless voids, and pit internal walls are kept clean (no shaft wall spikes).
- **Applied Hazard Decoration (Galleries & Grouping)**: Spikes are applied to surfaces in open air, never embedded in carved rock holes. The painted clearance is the movement envelope: it provably contains the Delver's body during required traversal (walking corridors are ≥ Delver height; jump vaults are ≥ jump apex + body). Ceiling strips therefore hang from *gallery* ceilings raised `ceiling_gallery_raise_tiles` rows (default 2) strictly above the **highest envelope top of their run** — untouchable during required movement at any corridor height (tall corridors from `max_floor_clearance_delver_heights` included), reachable only by optional jumps. Galleries are snug (strip + one end column per side), require solid rock to carve through plus a solid cap, never carve a cell adjacent to an existing spike (master theft), and skip actor columns. Strip lengths are sampled per gallery from `spike_group_min/max_tiles` so spikes appear grouped; wall spikes may mount the raised gallery end walls on the rows between the envelope top and the strip (`wall_spike_trap_odds`, per cell, and only alongside a strip). `ceiling_spike_trap_odds` gates gallery creation per eligible corridor run. Internal corners never show spikes on **both** adjoining surfaces: 4-adjacent spikes always share one orientation — gallery strips and end-wall spikes only occupy single-master cells, while pit corner cells point up with their row. Any spike left master-less collapses into solid rock (spike traps are `AttachedTile`s that need a neighboring platform).
