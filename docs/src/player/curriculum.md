...
* Arms after `E` focus episodes (`focus_episodes_between_passes` in `client/src/config.toml`), scheduling at most `K` priors with ~`R` episode slots each (review-only mix; budgeted cycles).
